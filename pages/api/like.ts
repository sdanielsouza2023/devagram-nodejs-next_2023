...
const likeEndpoint = async (req: NextApiRequest, res : NextApiResponse) =>{
    try{
        // como esta adcionando vamos usar o metod put para atualizar
        if(req.method === 'PUT'){
            const {id} = req?.query
            ...
            const indexDoUsuarioNoLike = publicacao.likes.findIndex((e : any ) => e.toString() === usuario._id)
        ...
        console.log(e)
        return res.status(500).json({erro:"Ocorreu erro ao curtir/descricao"})
    }
}
